...
const Code = ({
  ...
  const scrollToHighlightedCode = (lines = []) => {
    ...
    const scrollElem = doScrollWrapper
      ? wrapper.current
      : document.scrollingElement || document.documentElement

    const lineStartElem = d3
      .select(wrapper.current)
      .select(`#CodeLine-${lines[0] || 1}`)
      .node()
    let lineEndElem = d3
      .select(wrapper.current)
      .select(`#CodeLine-${lines[lines.length - 1]}`)
      .node()
    ...
    const height = doScrollWrapper
      ? wrapper.current.offsetHeight
      : window.innerHeight

    const doesFitOnScreen = lineEndY - lineStartY < height * 0.7
    const y =
      !doScrollToTop && doesFitOnScreen
        ? lineStartY +
          (lineEndY - lineStartY) / 2 -
          height / 2 +
          scrollElem.scrollTop
        : lineStartY + scrollElem.scrollTop - 35

    if (typeof scrollCancelFunction.current == "function")
      scrollCancelFunction.current()

    const documentScrollingElem =
      document.scrollingElement || document.documentElement
  ...
  useEffect(() => {
    ...
    scrollToHighlightedCode(markers[highlightedMarker])
  }, [highlightedMarker])
    ...
    steps.forEach((d, i) => {
      ...
      if (doKeepInitialLineNumbers) {
        ...
        if (doesOverlapRemovedLine) {
          ...
          runningRemovedLines = runningRemovedLines.filter(
            d => d > runningStartLine
          )
        }
      }
      if (!d.startsWith("//")) {
  ...
  const onExecuteEditedCodeLocal = () => {
    const codeBlocks = Array.from(wrapper.current.querySelectorAll("code"))
    ...
    <div {...props} className="Code__wrapper">
      {!!fileName && (
        <div className="Code__file">
          ...
          <div className="Code__file__name">{fileName}</div>
          ...
          <div className="Code__file__actions">
            ...
            <Tooltip
              ...
              contents="Copy code to my clipboard"
            >
              <ClipboardTrigger text={parsedCode} />
            </Tooltip>
          </div>
        </div>
      )}
      ...
      <div className="Code__run-buttons">
        {isEditing ? (
          <>
            <Button
              className="Code__run-button"
              onClick={onExecuteEditedCodeLocal}
            >
...
const CodeStep = React.memo(
  ({
    ...
    <div
      className={`CodeStep CodeStep--number-${number} CodeStep--is-${isExpanded ? "expanded" : "collapsed"}`}
      onClick={isExpanded ? () => {} : onToggle}
    >
      <div className="CodeStep__copyable-text">{`  // ${number}. ${name}`}</div>

      {!!isExpanded && <div className="CodeStep__close" onClick={onToggle} />}

      <div
        className="CodeStep__top"
        id={`CodeLine-${startLineIndex}`}
        onClick={onToggle}
      >
        {hasLineNumbers && (
          <div className="CodeLine__number">{startLineIndex}.</div>
        )}

        <div className="CodeStep__name">
          <div className="CodeStep__number">{number}.</div>
...
const CodeLines = React.memo(
  ({
    ...
    return code
      ...
      .slice(0, -1)
      .map((line, index) => {
        const isHighlighted = highlightedLines.includes(
          startLineIndex + index + 1
        )
        const markerIndex =
          markers &&
          markers.findIndex(list => list.includes(startLineIndex + index + 1))
...
const CodeLine = React.memo(
  ({ code, index, isHighlighted, markerIndex, isEditable, hasLineNumbers }) => {
    ...
    return (
      ...
      >
        {markerIndex != -1 && (
          <div className={`CodeLine__marker CodeLine__marker-${markerIndex}`} />
        )}
        {hasLineNumbers && <div className="CodeLine__number">{index + 1}.</div>}
        ...
        {isHovering && (
          <ClipboardTrigger className="CodeLine__copy" text={code} />
        )}
      </div>
    )
  }
